...
import { ScrapeData, PageContent, ContactInfo, StoreInfo } from './types.js';
...
class WebScraperService {
  ...
  // ============================================================================
  private extractEmails(bodyText: string): string[] {
  ...
  // ============================================================================
  private async extractContactInfoFromDOM(page: Page): Promise<any> {
    return await page.evaluate(() => {
      ...
      return {
        emails: Array.from(document.querySelectorAll('a[href^="mailto:"]'))
          .map(a => (a as HTMLAnchorElement).href.replace('mailto:', ''))
  ...
  // ============================================================================
  private scoreInstitutionalLink(url: string, text: string, context: string): number {
    let score = 0;
    ...
    if (productTerms.some(term => urlLower.includes(term))) return -10;

    const highPriority = ['contato', 'fale-conosco', 'sobre', 'empresa', 'institucional'];
    if (highPriority.some(term => urlLower.includes(term) || textLower.includes(term))) score += 5;

    const mediumPriority = ['privacidade', 'termos', 'politica', 'ajuda', 'faq'];
    if (mediumPriority.some(term => urlLower.includes(term) || textLower.includes(term))) score += 3;

    const lowPriority = ['trocas', 'devolucoes', 'envio', 'pagamento', 'carreiras'];
    if (lowPriority.some(term => urlLower.includes(term) || textLower.includes(term))) score += 1;

    if (contextLower.includes('footer') || contextLower.includes('institucional')) score += 2;
  ...
  // ============================================================================
  private extractInstitutionalLinks(page: Page, baseUrl: string): Promise<any[]> {
  ...
  private isInstitutionalLink(url: string, text: string): boolean {
    return this.scoreInstitutionalLink(url, text, '') > 0;
  }

  private getPageMarkdown($: cheerio.CheerioAPI): string {
  ...
  // 🚀 MÉTODO PRINCIPAL
  // ============================================================================
  async scrapeUrl(url: string, options?: any): Promise<ScrapeData> {
    this.visitedUrls.clear();
    const browser = await this.initBrowser();
    const institutionalPages: Record<string, PageContent> = {};
    ...
    const linksToVisit = mainPage.links
      .filter((link: any) => link.internal && link.score > 0 && !this.visitedUrls.has(link.url))
      .slice(0, 12);
  ...
  // 🛠️ SCRAPING INTERNO
  // ============================================================================
  private async internalScrape(browser: Browser, url: string, options?: any) {
  ...
  private extractSocialMedia($: cheerio.CheerioAPI): Record<string, string> {
    ...
    $('a[href]').each((i, el) => {
      ...
      else if ((href.includes('wa.me/') || href.includes('whatsapp.com/') || href.includes('api.whatsapp.com')) && !social.whatsapp) social.whatsapp = href;
    });
    return social;
  }
